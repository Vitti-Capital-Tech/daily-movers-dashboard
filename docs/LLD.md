...
**The snapshot chart's form is decided by the fitter** (`fitSnapshotChart`, `REPORT_LIMITS.snapshot*Points`). `columns` carries two to five points for one series and four for two, with a figure over every bar; `line` carries six to twelve and prints only the end figures, labelling alternate periods past eight. A `line` with fewer than six points is demoted to columns, and a series over its cap keeps its latest points, not its first. `asSnapshotChart` used to drop `form`, so no line ever reached the renderer. `niceTicks` now runs until a tick clears the maximum; stopping at `max + size / 2` left the top tick under the tallest value and drew it above the plot. Series colour comes from each series' `tone` (`unfavourable` coral, `favourable` mint, `neutral` cobalt then steel), not from its position, which had drawn revenue in coral on every falling day.

**The headline leads with the move.** Short name, direction and a whole-number move rounded from the share-move tile ("Tuas Sinks 16% Despite 277% Profit Jump…"), a "despite" framing when the stock went against the news, and no backstory shorthand. Section 9 of the writer prompt states it with the Tuas markup of 23 September 2026 as the counter-example; section 14 has the checker flag a headline that breaks it.
