...
│   │   │   ├── drafts/[id]/sources/ # Admin-only ZIP of every announcement the draft was written from
│   │   │   │   └── route.ts
...
│   │   │   ├── template.tsx     # The react-pdf document: one 960x540 sheet
...
**The document is ONE page, and the constraint is the layout rather than the page count.** Since 16 September 2026 the desk publishes a single sheet (PIA, `daily_movers` id 65), so the tool schema pins `pages` to `minItems: 1, maxItems: 1` of kind `snapshot`, `validateReportDoc` branches to a whole-document check that returns early (no cover-first, closing-last or four-page-minimum rules apply), and `sheetCount` omits the disclaimer sheet — a one-page report that prints as two pages is not a one-page report.

The failure mode inverted with the format. A long page in a deck wrapped onto a second sheet, which looked wrong and was therefore noticed; a long one-pager pushes its last risk card past the bottom edge and says nothing. So `fitPageBody` hard-caps every list on the snapshot — 4 tiles, 3 `whyItMoved`, 4 `whatChangesNow`, 5 timeline steps, 3 risks, with per-item character trims — rather than trusting a model that has been told the counts twice. A render returning one page is the evidence it fitted, since react-pdf paginates on overflow.

Risks survive the format change as the one structurally enforced editorial rule: `validateReportDoc` still refuses a snapshot with no risk cards, for the same reason it refused a deck with no risks page. `ONE_PAGE_DISCLAIMER` is transcribed verbatim from the published PDF, not derived from `DISCLAIMER_PARAGRAPHS` — shortening regulated text is a compliance decision, so it is a constant a human edits rather than a transformation code performs.
...
**The sheet is 960x540, not A4.** The template rendered A4 portrait on white until a generated draft was compared with what the desk actually publishes — the Focus Minerals note of 11 September 2026, a 16:9 deck on deep navy with a mint rule top and bottom, the house mark in the corner, a serif display line, and content in tiles, icon cards, dated timelines and before/now card tables. Next to it the portrait draft read as a typed memo. 16:9 rather than A4 landscape because the sheet is read on a screen far more often than it is printed, and the extra width is what lets four tiles sit in a row with air around them.

**Type size is set by the worst case.** The first NZK sheet left 59pt empty under the source line because that day'"'"'s copy was shorter than the caps allowed; enlarging the type to fill it would have overflowed the first draft that used its full allowance. `npm run report:preview -- stress` renders every list at its maximum count with every string at its `fit.ts` cap, and the type scale is whatever that fixture carries — currently ~6% above the original sizes with caps cut to match (76 chars a clause, 84 a risk, 36 a timeline step, 96 a headline, 155 a pull quote). The prompt states the same numbers, so the model writes to length instead of being truncated mid-word.

**The snapshot always carries a chart, and the timeline has its own band** (`SnapshotChartBand`, `PriceChartCard`, `TimelineBand`, `src/lib/report/price-band.ts`, `src/lib/market/price-history.ts`). The order follows the desk's CVB sheet: tiles, chart card, the two columns, *How we got here*, risks. No snapshot draft ever carried a chart while `timeline` was required and `chart` was its optional alternative. So `chart` is now in the tool's `required` list and `series` takes `minItems: 0`: the model sends `[]` when the filings hold no series, `asSnapshotChart` returns null for it, and `SnapshotBody` draws `PriceChartCard` from `ReportDoc.priceHistory` instead. `finishDraft` attaches that history (two years of daily closes, one `fetchDailyBars` request, never throws) after every rewrite and before render, so the stored document and the PDF carry the same series.

`layoutPriceBand` fits the window to the timeline's span: from the first parseable step, or 45 days back, to the move date or the last upcoming step within 120 days. It pads 4% either side and caps the window at 730 days. `reportPrice` is today's point. Each step goes on the close on or before its date. A step within 2.8% of the plot width of the previous one stacks a level, with a stem. Upcoming steps sit hollow on a dotted run from today. `TimelineBand` numbers its dots only when the price card is drawn, because only then do the numbers point at anything.

`ChartCard` is the shared frame: accent title with the note inline (wrapping rather than colliding with the legend), legend right, unit rotated in a 16pt gutter, footnote under. `SnapshotChartBand` draws `line` (the house default: dots and labels on every point up to six, ends only past that, the higher series labelled above and the lower below) or `columns`. It adds 20% headroom past the extremes so the printed figures clear the category labels. `PriceChartCard` fits its axis to the data rather than zero, with at least three tick labels. `priceLabel` takes its decimals from the tick step, so a 2.5 step prints `$2.50`, not `$3`. Series colour comes from each series' `tone` (`unfavourable` coral, `favourable` mint, `neutral` cobalt then steel), not its position, which had drawn revenue in coral on every falling day. `fitSnapshotChart` draws any chart whose categories all match `PERIOD_LABEL` (`FY26`, `1Q27`, `2H25`, `Jul-26`, `2025`) as a line, whatever form the model sent, because the model sent quarterly NOI as columns on 24 September 2026. A line's axis is fitted to the data with 28% headroom rather than to zero (zero stays in range, dashed, when the series cross it), each point is a haloed ring, and a lone series gets a 12% fill. It caps a line at 12 points and columns at 5, or 4 with two series, keeping the latest points; `asSnapshotChart` used to drop `form`, so no line ever reached the renderer. `niceTicks` runs until a tick clears the maximum, where stopping at `max + size / 2` had left the tallest value above the plot. Fitting both bands on one sheet removed the "ONE-PAGE SNAPSHOT" label, set the headline at 27.5pt and the chart plot at 92pt. The headline is never cut: `headlineSize` steps it down (27.5 / 24 / 21 / 19pt at 100 / 125 / 150 characters) so it stays whole in two lines, and the fitter's 170-character cap only stops a runaway.

**Over-budget lines are rewritten before they are cut** (`SNAPSHOT_CAPS`, `SNAPSHOT_TARGETS`, `snapshotOverruns`, `applySnapshotRewrites` in `fit.ts`; `shortenOverruns` in `mover-draft.ts`). `writeReport` normalises the pages, then `shortenOverruns` sends each line over budget (addressed as `whyItMoved.0`, `timeline.2`, `risks.1.text`, `chart.title`, `headline`...) to a `shorten_lines` tool call without evidence. That call has no thinking, `max_tokens` 4000 and up to two rounds. A rewrite is accepted when it is within `max`, has no ellipsis, and every figure in it (`/\d[\d,]*(\.\d+)?/`, commas stripped) appears in the original. Round two carries the refused attempt and its length. Failures and refusals fall back to `fitReportPages`, which still trims, and each case is logged. Its usage is added to the draft's.

**Leftover height is distributed.** `snapBody` is `justifyContent: space-between`, so slack goes to the gaps between bands rather than pooling under the last one; the heading trio is wrapped in its own View so the distribution does not push the label, company name and headline apart. Measured across the stress fixture, the PIA fixture and a real draft, all three now end at the same point on the sheet.

**The `snapshot` layout is a fixed grid, not a flow.** Each band in `SnapshotBody` has a stated height and they are budgeted to add up to the 540pt page once the chrome and the footer are taken out (~430pt of content). That is why the style block reads as a set of magic numbers: they are the contract `fit.ts` cuts content to meet. `paddingBottom: FOOTER_HEIGHT + 18` on the page reserves the footer, so body content flows above it rather than under it.

**Colour carries meaning, never decoration** (`PALETTE`, `deckTheme`, `pageAccent`). Navy is the ground, with tiles a lighter navy so they read as raised rather than outlined and the footer band darker so the page has a floor. The **accent** marks the finding — the eyebrow, the highlighted tile, the conclusion band, the closing pull quote — so a reader who follows only the accent gets the argument; and it is **mint on a rising day, coral on a falling one**, because a Daily Mover is about direction before it is about anything else. **Cobalt** is a fact without a verdict, and stays cobalt in both. Body copy is one off-white and one grey and nothing else, because coloured body text on a dark ground makes a slide read as a warning label.

The theme travels as a **prop**, not through React context: this module is reachable from a Server Component (the cron route imports the renderer), and `createContext` anywhere in that import graph is a Next.js build error. It is the kind of constraint worth writing down, because context is the obvious way to write this and it fails at build rather than at review.
...
**Layout is verified by looking at the rendered page.** A spacing bug looks plausible in the style object and wrong on the page: the KPI card once inherited the page's 1.5 line height and put a label's baseline **8pt** below a 21pt number's — inside its descender depth, so any value containing a descender collided outright. `npm run report:preview` renders the template with no API call: `-- snapshot` for the one-pager, `-- stress` / `-- stress-columns` / `-- stress-line` / `-- stress-price` / `-- stress-timeline` for the worst case of each chart form and the timeline, `-- fixture` for every legacy deck page kind, or `-- <draftId>` for a stored draft. The snapshot fixture holds the **real published PIA copy** rather than invented text, because the only question the preview has to answer is whether the desk's actual volume of copy fits on one sheet, and a fixture written to suit the grid cannot answer it. It bundles with esbuild first because `@react-pdf/hyphenate` declares only an `import` condition for its language subpaths, so a direct `tsx` run raises `ERR_PACKAGE_PATH_NOT_EXPORTED` on `./en-us`; Next.js never hits it because it bundles the renderer itself.
...
2. **Screen Criteria — and they persist**: Reads `minTurnover`, `minMarketCap`, `minAbsChangePct`, `perSide`, clamps each into `SCREEN_LIMITS`, then **saves them** via `saveScreenCriteria` (`lib/drafts/screen-settings.ts`) into the `screen_settings` singleton (`id = 1`). `generateDraft` resolves its screen as `options.criteria ?? await loadScreenCriteria()`, so the 06:00 scheduled run — which passes no criteria — picks up whatever the desk last submitted here. Before this the controls applied to one run and evaporated, which made the Studio's most visible settings inert against the thing they looked like they configured. The trade is that a one-off experiment is not one-off: widening the screen to look at a $1m-cap board leaves tomorrow's scheduled run screening that way until someone sets it back, which is why the form renders the stored row rather than the compiled defaults. `DEFAULT_SCREEN` stays the fallback for a missing row *and* for a failed read — both `loadScreenCriteria` and `saveScreenCriteria` swallow their errors, because a settings lookup must never be the reason a draft doesn't happen.
...
3. **Candidate Shortlist** — one announcements lookup per screened mover (concurrency 6, interleaved across gainers and losers, capped at 40). Movers with no price-sensitive filing that session are dropped as unexplainable. The screen defaults to **no turnover floor / $5m market cap / 25 rows a side**, after $500k/$20m and then $1m/$75m. The turnover gate went when the run moved to 10:30 Sydney — turnover is cumulative from the open, so a $1m floor thirty minutes in selects for whatever traded first and empties the board on a quiet morning. `passesScreen` still rejects a *null* turnover, because an unknown figure is not evidence of a liquid stock. Note the coupling: `CANDIDATE_LOOKUP_LIMIT` truncates the two boards interleaved, so it must stay at or above `perSide * 2` — at the old cap of 40 a `perSide` of 25 silently collapsed back to 20 a side. Each candidate is printed with its turnover as a share of market capitalisation — a large move on 8% of the register changing hands is a speculative blow-off, on 0.4% it is a re-rate, and the ratio is the cheapest way to tell them apart.
...
9. **`renderReportPdf()`** — validates document structure. For a `snapshot` document that means exactly one page carrying a headline, at least three KPI tiles, two `whyItMoved` points, one `whatChangesNow` point, two dated steps, at least one risk card and a closing line; the deck rules (cover first, closing last, a risks page present, four to five pages, charts carrying conclusions) still apply to stored multi-page documents. Then `@react-pdf/renderer` → `Buffer`, uploaded to `drafts/<TICKER>/<date>-daily-mover-<random>.pdf`.
...
2. **Timezone Gate**: the trigger is Supabase `pg_cron`, not Vercel Cron — `vercel.json` has no `crons` key, and `drizzle/cron-setup.sql` holds one job at `30 0 * * 1-5` UTC (06:00 IST), calling the route through `pg_net` with the cron secret from Vault. Vercel Hobby was dropped for precision: ±59 minutes, measured at ~29 minutes late daily. One schedule replaces the old pair because India has no DST, so 06:00 IST is 00:30 UTC year-round and there is no Sydney DST switch to straddle. The handler still proceeds only if Sydney local time falls between `MARKET_OPEN_MINUTES` (10:00) and 15:00. That 10:00 floor is load-bearing: 00:30 UTC is 10:30 Sydney under AEST, so the former 11:00 floor would have rejected the intended firing for six months of every year. Dedupe lives in the partial unique index rather than the clock, so a second caller (a Vercel cron added back as a fallback, a manual `select public.trigger_daily_mover()`) is a no-op and needs no code change. `?force=1` skips the window check alone, for re-running a missed session.
