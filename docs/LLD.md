...
**The snapshot chart's form is decided by the fitter** (`fitSnapshotChart`, `REPORT_LIMITS.snapshot*Points`). `columns` carries one or two series of two to four points with a figure over every bar; `line` carries six to twelve and prints only the end figures, labelling alternate periods past eight. A `line` with fewer than six points is demoted to columns, and a series over its cap keeps its latest points, not its first. `asSnapshotChart` used to drop `form`, so no line ever reached the renderer. `niceTicks` now runs until a tick clears the maximum; stopping at `max + size / 2` left the top tick under the tallest value and drew it above the plot.
