...
| --- | --- | --- |
| `cover` | What happened, and how far did it move | First page only. Full-bleed navy band, then the headline and exactly two hero KPI cards. The move card is tinted by direction from `ReportDoc.movePct`. |
| `narrative` | What does this business do / what changed | 2-5 paragraphs, optional small-caps callouts. |
| `kpis` | What the numbers were | 3-4 big-number cards; four go two-up over two rows, because a quarter-width card is ~90pt and narrower than labels like "CASH CONSIDERATION". |
| `entities` | Where does the money come from | Segments, geographies, acquisitions, each with a dense stat line. Often better as a `chart` with `type: "bars"`. |
| `chart` | What direction is this heading | One series plus a **required** conclusion line. See below. |
| `market-vs-reality` | Why did a record result sell off | Fixed three-block layout: headline / market focus / what really matters. Its own kind rather than three callouts, because the gap between the announcement and the reaction is the whole point on those days and a fixed layout stops it being written as another narrative page. |
| `comparison` | What changed since last time / consensus vs actual | Four-column table. One layout serves both because they are the same shape; `columns` names which. |
...
| `closing` | Where does this stand | Last page only. The house sign-off is appended by the renderer, never model-written — a model asked to end on a set phrase paraphrases it about one time in five, as with the disclaimer. |

**Charts carry a conclusion or they do not ship.** `validateReportDoc` rejects a `chart` page without its one-line "what to notice", and `normalisePage` drops one that arrives without a plottable series. Only two forms exist: `columns` for a series over time and `bars` for named categories whose labels will not fit under a column. Deliberately not a general charting layer — every insight the desk has needed is one series of labelled magnitudes, and a second axis or a scatter would be new ways to produce a chart nobody can read.
...
**Colour carries meaning, never decoration** (`PALETTE`, `pageAccent`). The first template was navy on white and nothing else: defensible printed, unfinished on screen, with nothing to tell a reader what kind of page they were on. Teal marks the analytical pages, gold the pages about people and process, and green/wine-red are reserved for direction and appear only where direction is the point. All are deep enough to survive a greyscale printer as distinguishable tones and none is the saturated red/green of a trading screen, which would read as a house view on a document that must not carry one. Body type stays black on white — colour goes into rules, bands, card borders and tinted panels only.

**Layout is verified by reading the rendered PDF back.** Text baselines and font sizes are extracted with `pdfjs` and checked, because a spacing bug looks plausible in the style object and wrong on the page: the KPI card inherited the page's 1.5 line height and put a label's baseline **8pt** below a 21pt number's — inside its descender depth, so any value containing a descender collided outright.
