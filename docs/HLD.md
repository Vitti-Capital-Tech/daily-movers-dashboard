...
5. **Mover Studio — Autonomous Daily Mover Drafting (`/mover-studio`, admin only)**:
   - Scheduled each weekday at 06:45 IST, which is 11:15 in Sydney — about an hour into the session (Vercel Cron, twice-daily UTC firing with an in-handler timezone gate, so daylight saving needs no change; `maxDuration` is 300s, the Hobby ceiling, and the optional stages are budgeted against it — see `CHECK_DEADLINE_MS`). On the Hobby plan the firing time is approximate to within an hour, so 06:45 is the earliest it runs rather than the time it runs.
