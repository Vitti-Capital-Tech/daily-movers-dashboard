...
| **AI Drafting & Verification Engine** | **Anthropic Claude Sonnet 5** | Selects the day's subject, writes the report as typed page blocks, and then re-reads it against the same filings as the Accuracy Gate. Opus 5 is one environment variable away (`ANTHROPIC_DRAFT_MODEL`) and is the lever for the failures that matter — a figure that felt correct, conditional money written as certain — at about 2.5x the cost. |
...
5. **Mover Studio — Autonomous Daily Mover Drafting (`/mover-studio`, admin only)**:
   - Scheduled each weekday at 06:00 IST, which is 10:30 in Sydney — half an hour into the session. The trigger is **Supabase `pg_cron` + `pg_net`**, not Vercel Cron: one job at `30 0 * * 1-5` UTC (`drizzle/cron-setup.sql`), because Vercel's Hobby plan schedules with ±59 minutes of precision and this job landed ~29 minutes late every day. One schedule suffices where Vercel needed two, since India has no daylight saving and 06:00 IST is 00:30 UTC year-round. The in-handler Sydney gate still runs and opens at the 10:00 bell rather than 11:00, because the AEST firing lands at 10:30. `maxDuration` is 300s, the Hobby ceiling, and the optional stages are budgeted against it — see `CHECK_DEADLINE_MS`.
   - Computes both sides of the ASX movers board from the exchange's company directory plus the market provider's session moves, applies a market-cap screen (no turnover floor / $5m by default, 25 rows a side), and drops any mover with no price-sensitive filing that session. The turnover floor was removed when the run moved to 10:30 Sydney: turnover accumulates from the open, so a dollar floor that far forward selects for whatever traded first rather than for liquidity. The figure is still computed and shown to the model as a share of market cap.
   ...
   - Emits the report's typed page blocks and the `daily_movers` columns in one structured call, bounded to **exactly one page of kind `snapshot`** — the single 16:9 sheet the desk publishes. That page carries the company name, a headline, four KPI tiles, two numbered columns (*Why It Moved* / *What Changes Now*), a chart card under the tiles (the model's series when the filings hold one, otherwise the share price from the exchange feed with the timeline steps marked on it), a *How We Got Here* dated timeline, three *Key Risks Remaining* cards, a closing pull quote and a source line naming the filings. The multi-page deck kinds (charts, comparison tables, segment blocks, management, scorecard, prose, closing) are retained so stored drafts still render, but are no longer emitted.
   ...
   - Renders the report to PDF with `@react-pdf/renderer` — a single 960x540 sheet on deep navy, accented green on a rise and red on a fall, with the house mark inlined as a data URI and the short compliance line in the footer in place of a separate disclaimer sheet — into a `drafts/` storage prefix, and presents it for approval with the archive fields editable, the report shown inline, and every announcement read listed with the cited ones marked.
