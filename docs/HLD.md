...
5. **Mover Studio — Autonomous Daily Mover Drafting (`/mover-studio`, admin only)**:
   ...
   - Renders the report to PDF with `@react-pdf/renderer` — a 16:9 slide deck on deep navy, accented green on a rise and red on a fall, with the house mark inlined as a data URI — into a `drafts/` storage prefix, and presents it for approval with the archive fields editable, the report shown inline, and every announcement read listed with the cited ones marked.
